...
`{"nodename":function, ...}`.
 ...
 - `cmdset_mergetype` (str): This is one of "Replace" (default) or "Union" (see [CmdSets](Command- Sets). The first makes the menu cmdset exclusive - the user has no access to any other commands while in the menu. "Union" layers the menu on top of previous commands instead (and may overload them, so be careful as to what to name your menu entries in this case).
