...
def is_typing_setup(session, *args, **kwargs):
    ...
    live_report_keywords = [kw for cmd in live_report_commands for kw in (cmd.key, *cmd.aliases)]

    matchset = set(live_report_keywords)

    live_report_keywords += [
        nick.value[2]  # this is the 'nick'
        for nick in session.puppet.nicks.all()
        if nick.value[3].split(" ")[0] in matchset  # spaced keywords
        or nick.value[3][0] in matchset  # handle things like ', ", :
    ]
